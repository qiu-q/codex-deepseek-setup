...
- 可选启用轻量 Mihomo 网络辅助：用户自行粘贴有权使用的 HTTPS 订阅，运行时优先从项目自有 HTTPS 镜像下载并校验固定 SHA-256，GitHub 官方 Release 仅作备用；启用后自动弹出节点选择窗口，可测试所选节点延迟、切换具体节点或继续使用自动测速，启用、停用和恢复均有可见进度。
- 订阅下载使用 Clash/Mihomo 兼容标识，并按实际响应内容识别配置；即使订阅服务错误标记为 `text/html`，只要内容确实是 Clash YAML 仍可导入，真正的网页内容仍会被拒绝。
- 窗口右下角常驻“查看日志”和“复制日志”；操作失败时入口会自动标红，日志记录安全的错误代码、HTTP 状态、下载源、异常类型和各阶段进度，并自动隐藏 API Key、Bearer 凭据以及 URL 查询参数和片段。
...
4. 进入 DeepSeek 页面后按分步弹窗完成登录、实名认证、充值和 Key 创建；弹窗可随时关闭，并可用“查看操作引导”重新打开。如果网络确实需要辅助，可先在可选区域填写自己的 HTTPS 订阅并启用；节点载入后会自动显示选择窗口，关闭窗口则保留“自动选择”，稍后可点“选择节点”重新打开。不需要时直接跳过。粘贴 API Key 后点击“验证并配置 Codex”。
...
遇到订阅或 Mihomo 下载失败时，点击窗口右下角“复制日志”即可粘贴给技术人员；也可以点“查看日志”后导出到桌面。每次启动的原始脱敏日志保存在 `%LOCALAPPDATA%\CodexDeepSeekSetup\Logs`，即使磁盘写入失败仍可从当前窗口复制内存日志。
...
- 节点切换使用只监听 `127.0.0.1` 的 Mihomo 控制接口，并由每台电脑随机生成至少 128 位控制密钥；接口不会向局域网开放，选择结果由 Mihomo 保存在当前用户数据目录。
